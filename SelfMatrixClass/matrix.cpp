...
void matrix::appendCols(const matrix& augment)
{
    if (isempty || augment.isempty)
        throw Error<2>();
    if (height != augment.height)
        throw Error<4>("Augment matrix must have the same height as the matrix to be widened\n");
    this->width += augment.width;
    for (int i = 0; i < height; i++)
        pmtx[i].insert(pmtx[i].end(), augment.pmtx[i].begin(), augment.pmtx[i].end());
    return ;
}

void matrix::appendRows(const matrix& augment)
{
    if (isempty || augment.isempty)
        throw Error<2>();
    if (width != augment.width)
        throw Error<4>("Augment matrix must have the same width as the matrix to be heightened\n");
    this->height += augment.height;
    Row* temp = pmtx;
    pmtx = new Row[height];
    for (int i = 0; i < height-augment.height; i++)
        pmtx[i] = temp[i];
    for (int i = height - augment.height; i < height; i++)
        pmtx[i] = augment.pmtx[i - height + augment.height];
    delete [] temp;
    return ;
}
...
Row operator +(const Vec& v1, const Vec& v2)
{
    if( v1.empty() || v2.empty() )
        throw Error<1>();
    int l0 = v1.size(); int l1 = v2.size();
    if(l0 != l1) 
        throw Error<3>();
    else{
        Vec v(v1);
...
const Row& matrix::operator [] (int rowi)
{
    if (isempty)
        throw Error<2>();
...
int findptv(int col_in, int height, Row* p_matrix, int row_start = 0)
{
    ...
    {   
        if( p_matrix[i][col_in] > 1e-7 || p_matrix[i][col_in] < -1e-7)
            return i;
        else
            p_matrix[i][col_in] = 0;
    }       
...
Vec matrix::rowSubstract(Plus rowi, Plus row_substractor, double ratio)
{
    ...
    {
        for(int j = 0; j < width; j++){
            if(pmtx[rowi][j] > -1e-7 && pmtx[rowi][j] < 1e-7)
                pmtx[rowi][j] = 0;
...
Vec matrix::colMultiply(Plus coli, double lambda)
{
    ...
    else
    {
        ...
        {
            pmtx[i][coli] *= lambda;
            if(pmtx[i][coli] > -1e-7 && pmtx[i][coli] < 1e-7)
                pmtx[i][coli] = 0;
...
matrix matrix::inverse(void) const
{
    ...
    //         if(i == find_start)
    //             continue;
    //         if( A.pmtx[i][j] > 1e-7 || A.pmtx[i][j] < -1e-7) {
...
matrix matrix::kernel(void) const
{
    ...
    for (; m < mtxTemp.height; m++)
    {
        ...
        {
            if (mtxTemp.pmtx[m][n] > 1+1e-7 || mtxTemp.pmtx[m][n] < 1-1e-7)
            {
                continue;
            }
    ...
    for (int coli = 0; coli < mtxTemp.width; coli++)
    {
        ...
        for (; rowi != -1; rowi = findptv(coli, mtxTemp.height, mtxTemp.pmtx, rowi+1))
        {
            ...
            {   
                if (mtxTemp.pmtx[rowi][colj] < 1+1e-7 && mtxTemp.pmtx[rowi][colj] > 1-1e-7)
                {
...
Vec matrix::specialSolution(Vec b) const
{
    ...
    {
        if (mtxb.pmtx[rowi][0] < -1e-7 || mtxb.pmtx[rowi][0] > 1e-7)
        {
            noSolution = true;
        }
    }
    ...
    for (; m < A.height; m++)
    {
        for (; n < A.width ; n++)
        {
            if (A.pmtx[m][n] < 1-1e-7 || A.pmtx[m][n] > 1+1e-7)
            {
...
matrix matrix::congruentTransform(const matrix & transMat) const
{
    if (isempty || transMat.isempty)
        throw Error<2>();
    if (transMat.height != width || width != height)
        throw Error<4>("Congruent transform requires square matrix!\n");
    if (!transMat.invertible())
        Warning<0>("The transform matrix should be invertible, which it isn't.\n");
    return transMat.transpose()*(*this)*transMat;
}

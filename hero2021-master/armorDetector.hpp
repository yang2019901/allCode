...
struct ArmorLightExtraInfo
{
    ...
    ArmorLightExtraInfo(vector<Point> _poly, float _colorFeature, int _tag) //构造函数
    {
        ...
        len = arcLength(poly, true);
        tag = _tag;
        rr = minAreaRect(_poly);
       if (abs(rr.angle) > 45)
        {
            rr.angle += 90;
...
//LOG(A)
        //vector<vector<Point>> light_polys;
        light_poses.clear();
        ...
        for (int i = 0; i < contours.size(); i++)
        {
            ...
            ArmorLightExtraInfo al(ct, 1, light_poses.size());
            if (al.rr.size.width / al.rr.size.height > LightAspectRatioThresh) continue;    //删除长宽比不合适的矩形
            if (al.len > 800 || al.len < 30) continue; //删除周长过长或者过短的矩形
            if (al.rr.size.height < 22 &&  al.rr.size.width > 22) {continue;} //删除长宽不合适的矩形
            ...
            bound.height += 2 * TeamColorCheckDistance;
            ...
            {
                ellipse(org, al.rr, Scalar(0,255,0));
                circle(org, al.rr.center,al.rr.size.height / 2,Scalar(255,0,255),2);
            }
        }
        sort(light_poses.begin(), light_poses.end(), CompareLight); //按照灯条颜色特征进行排序

//LOG(B)
        // reset tag to the sorted index if DEBUG
        if(DEBUG_MODE) 
            FOREACH(i,light_poses.size())
            light_poses[i].tag = i; //给每个灯条标号

        int actualSize = min((int)light_poses.size(), LightTopNumber);
        // make pairs
        //cout << "--Assess pairs" << endl;
        combination.clear();
//LOG(C)
        // parallel calculation 先配好对，后期并行计算任务分配更均匀
        FORPAIR(i,j,actualSize) combination.push_back(ArmorResult(light_poses[i], light_poses[j],0,false));
        #pragma omp parallel for
        FOREACH(i,combination.size())
                combination[i].score = CheckPair(combination[i].leftLight,combination[i].rightLight,combination[i].smallArmor);
//LOG(D)
        sort(combination.begin(), combination.end(), CompareResult);
        // pick pairs
        result.clear();
        bool *tags = new bool[light_poses.size()];
        FOREACH(i, light_poses.size())  tags[i] = false;
        FOREACH(i, combination.size())
        {
            if (combination[i].score < PairScoreThresh) break;
            if (tags[combination[i].leftLight.tag] || tags[combination[i].rightLight.tag])
                continue;
...
//LOG(E)
        //drawContours(org, light_polys, -1, Scalar(255, 0, 0));
        if (DEBUG_MODE && allowDebug)
        {
            DEBUG_DISPLAY(binary);
            DEBUG_DISPLAY(teamColor);
            FOREACH(i, result.size())
            {
                circle(org, result[i].center, 4, Scalar(0, 255, 0), 2);
                // draw hist
                Mat hist,graph;
                GetHist(ArmorCenterAreaGrey(result[i]),hist,graph);
                line(graph,Point(HistMinThresh,0),Point(HistMinThresh,200),Scalar(155));
                line(graph,Point(HistMaxThresh,0),Point(HistMaxThresh,200),Scalar(155));
                DEBUG_DISPLAY(graph);
            }
            char cstr[10];
            FOREACH(i,light_poses.size())
            {
                sprintf(cstr,"%d",light_poses[i].tag);
                putText(org,cstr,light_poses[i].rr.center,FONT_HERSHEY_SIMPLEX,1.0,Scalar(0,0,0),2);
            }
        }
    }
...
//LOG(a)
        ...
        float len = Length(dt, Point());
        if (len * 0.8 > lt1.len + lt2.len)
            return -1;
        if (len - (lt1.rr.size.width + lt2.rr.size.width) / 2 < (lt1.rr.size.width+lt2.rr.size.width) * 0.3)
            return -1;
        // abundon if veritcal
        if (abs(dt.y/(dt.x+0.001)) > 0.8) return -1;

//LOG(b)
        float ang1, ang2, score = -PolyMatchScoreRate * JudgeLights(lt1,lt2);
        //cout << score << " ";
        ang2 = (float)atan2(dt.y, dt.x) * Rad2Deg;

        ang1 = CircularTo(lt1.rr.angle - ang2, -45, 45) / 45;
        if (abs(ang1) < 0.7)
            score += LightPairAngleScoreRate * (1 - ang1*ang1);
        //cout << score << " ";
...
//LOG(d)
    ...
    {
        split(src, spl);
        //addWeighted(spl[targetArmorColor], 1, spl[2 - targetArmorColor], -1, 0, teamColor);
        ...
        //        teamColor = dilated & teamColor;
#endif
    }

    int CountTeamColor(Rect r)
    {
        MakeRectSafe(frame,r);
        int count = 0;
        FOREACH(i, r.height)
        {
            uchar* p = teamColor.ptr<uchar>(i + r.y) + r.x;
            FOREACH(j, r.width)
                count += p[j];
        }
        return count / 255;
...
protected :
    ...
    static float PointDistanceToPoly(Point p, vector<Point> poly)
    {
        ...
        for (int i = 0; i < poly.size(); i++)
        {
                t = PointDistanceToLine(p, poly[i], poly[(i + 1) % poly.size()]);
                mindis = min(t, mindis);
        }
        return mindis;
    }
    ...
    static float PolyDistanceToPoly(vector<Point> poly1, vector<Point> poly2)
    {
        ...
        {
            double weight = Length(poly1[i], poly1[(i + 1) % poly1.size()]) + Length(poly1[i], poly1[(i + poly1.size() - 1) % poly1.size()]);
    ...
    {
        return (PolyDistanceToPoly(l1.poly, l2.poly) + PolyDistanceToPoly(l2.poly, l1.poly)) / max(l1.len ,l2.len);
    }
